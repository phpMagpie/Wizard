 ...
 */		
	function process($step) {
		if (isset($this->controller->params['data']['Cancel'])) {
		...
		if (isset($this->controller->params['data']['Draft'])) {
		...
		if (empty($step)) {
			...
			if ($this->_validStep($step)) {
				...
				if (!empty($this->controller->data) && !isset($this->controller->params['data']['Previous'])) { 
					$proceed = false;
				...
				} elseif (isset($this->controller->params['data']['Previous']) && prev($this->steps)) { 
					$this->redirect(current($this->steps));
